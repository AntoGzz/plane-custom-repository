...
def service_importer(service, importer_id):
    try:
        ...
        if getattr(settings, "PROXY_BASE_URL", None):
            ...
            # Community / local path (no Silo proxy): run import in-process
            imported = {}
            if service == "github":
                from plane.utils.importers.github import run_github_import

                imported = run_github_import(importer)
            elif service == "jira":
                from plane.utils.importers.jira_import import run_jira_import

                imported = run_jira_import(importer)
            importer.imported_data = imported
