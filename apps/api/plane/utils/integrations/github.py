...
def get_jwt_token():
    ...
    raw_secret = os.environ.get("GITHUB_APP_PRIVATE_KEY", "") or ""
    # Support keys stored with literal \n in env files
    raw_secret = raw_secret.replace("\\n", "\n")
    secret = bytes(raw_secret, encoding="utf8")
    current_timestamp = int(datetime.now().timestamp()) - 60
    expiry = current_timestamp + 9 * 60
    payload = {
